...
function parse_galaxies_results() {
	RESULTS_DIR=$1
	TOTAL_LOGS=$2
	echo "RUN, TRANSFER/SEC, REQUESTS/SEC, AVG_LATENCY, ERRORS" > ${RESULTS_DIR}/Throughput.log
	for (( iteration=1 ; iteration<=${TOTAL_LOGS} ;iteration++))
	do
		RESULT_LOG=${RESULTS_DIR}/wrk-${inst}-${iteration}.log
		transfer=`cat ${RESULT_LOG} | grep "Transfer/sec" | awk '{print $2}'`
		requests=`cat ${RESULT_LOG} | grep "Requests/sec" | cut -d " " -f2`
		connectionerror=`cat ${RESULT_LOG} | grep "Socket" | awk '{print $4}' | tr -d ","`
		reseterror=`cat ${RESULT_LOG} | grep "Socket" | awk '{print $6}' | tr -d ","`
		timeouterror=`cat ${RESULT_LOG} | grep "Socket" | awk '{print $7}' | tr -d ","`
		sum=$((connectionerror+reseterror+timeouterror))
		N="3"
		errors=`echo $sum / $N | bc -l`
		latency=`cat ${RESULT_LOG} | grep "Latency" | awk '{print $2}'`
		echo "${iteration},    ${transfer},     ${requests},     ${latency},    ${errors}" >> ${RESULTS_DIR}/Throughput.log
	done
}
