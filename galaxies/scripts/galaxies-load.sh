...
do	
	...
	do
		...
		LOAD_DURATION="60s"
		REQUEST_STATE="2000"
		CONNECTIONS=$(( 1000*iter ))
	
		case "${CLUSTER_TYPE}" in
			docker)
				cmd="./wrk2.sh --threads=${LOAD_USERS} --connections=${CONNECTIONS} --duration=${LOAD_DURATION} --rate=${REQUEST_STATE} http://${IP_ADDR}:${GALAXIES_PORT}/galaxies"
				;;
			minikube)
				cmd="./wrk2.sh --threads=${LOAD_USERS} --connections=${CONNECTIONS} --duration=${LOAD_DURATION} --rate=${REQUEST_STATE} ${IP_ADDR[inst-1]}/galaxies"
				;;
			openshift)
				cmd="./wrk2.sh --threads=${LOAD_USERS} --connections=${CONNECTIONS} --duration=${LOAD_DURATION} --rate=${REQUEST_STATE} http://${IP_ADDR[inst-1]}/galaxies"
				;;
		esac
