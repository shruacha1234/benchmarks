...
function usage() {
	echo
	echo "Usage: $0 -s BENCHMARK_SERVER -e RESULTS_DIR_PATH [-u WRK_LOAD_USERS] [-d WRK_LOAD_DURATION] [-w WARMUPS] [-m MEASURES] [-i TOTAL_INST] [--iter=TOTAL_ITR] [-r= set redeploy to true] [-n NAMESPACE] [-g GALAXIES_IMAGE] [--cpureq=CPU_REQ] [--memreq=MEM_REQ] [--cpulim=CPU_LIM] [--memlim=MEM_LIM] "
	exit -1
}

# Iterate through the commandline options
while getopts s:e:u:d:w:m:i:rg:n:-: gopts
do
	case ${gopts} in
	-)
		case "${OPTARG}" in
	...
	e)
		...
		;;
	u)
		WRK_LOAD_USERS="${OPTARG}"
		;;
	d)
		WRK_LOAD_DURATION="${OPTARG}"
		;;
	w)
		WARMUPS="${OPTARG}"		
		;;
...
if [ -z "${WRK_LOAD_USERS}" ]; then
	WRK_LOAD_USERS=400
fi

if [ -z "${WRK_LOAD_DURATION}" ]; then
	WRK_LOAD_DURATION=300
fi
...
CPU_MEM_DURATION=`expr ${WRK_LOAD_DURATION} + 10`
